...
* Cohen's Benchmarks für kleine, mittlere und große Effekte liegen bei $U_1=.15$, $U_1=.33$ und $U_1=.47$.
:::
...
Die Grundidee von Cliff's d ist jeden Punkt einer Gruppe $x_i$ mit jedem Punkt der anderen Gruppe $y_i$ zu vergleichen und zu entscheiden, ob $\color{#8cd000}{x_i < y_i}$, $x_i = y_i$ oder $\color{#d77d00}{x_i > y_i}$ gilt
...
$$= \frac{\color{#d77d00}{\#(x_i > y_i)} - \color{#8cd000}{\#(x_i < y_i})}{\#X \cdot \#Y}$$

$$= \frac{\color{#d77d00}{11} - \color{#8cd000}{1}}{16}$$
$$= .625$$ 
...
data_expanded <- 
  ...
  gather(ort_y, y) |> 
  left_join(data_rawplot |> select(y, x))
...
::: {.incremental}
* Cliff's $d(X,Y) := \frac{\#(x_i > y_i) - \#(x_i < y_i)}{\#X \cdot \#Y}$ 
